...
static li_measures_status_t g_measures_status =
    {
    .last_error = LI_MEASURES_STATUS_NOT_READY,
        .error_count = 0U,
        .is_ready = 0U,
        .is_running = 0U,
};
...
static void LI_measures_set_error(status_measures_t error)
{
    __disable_irq();
...
status_measures_t LI_start_adc_dma(void)
{
    ...
    {
        LI_measures_set_error(LI_MEASURES_STATUS_HW_ERROR);
        return LI_MEASURES_STATUS_HW_ERROR;
    }
    ...
    __disable_irq();
    g_measures_status.last_error = LI_MEASURES_STATUS_OK;
    g_measures_status.is_ready = 1U;
    g_measures_status.is_running = 1U;
    __enable_irq();

    return LI_MEASURES_STATUS_OK;
}
...
status_measures_t LI_measures_on_adc_conv_cplt_isr(ADC_HandleTypeDef *hadc)
{
    ...
    {
        LI_measures_set_error(LI_MEASURES_STATUS_INVALID_ARG);
        return LI_MEASURES_STATUS_INVALID_ARG;
    }
...
status_measures_t LI_measures_get_latest_sample(uint16_t *out_sample)
{
    ...
    {
        LI_measures_set_error(LI_MEASURES_STATUS_NOT_READY);
        return LI_MEASURES_STATUS_NOT_READY;
    }
